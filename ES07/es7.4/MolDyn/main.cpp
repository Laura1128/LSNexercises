...
int main(){ 
  ...
  for(int istep=1; istep <= nstep; ++istep){
     ...
     //   ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
        Accumulate(); 
        nconf += 1;
	if(nconf==nstep/10 && restart==true){
		ConfPreFinal();
	}
     }
  }
...
void Input(void){ //Prepare all stuff for the simulation
  ...
  if (restart == true) {
	cout << "The re-start option is on" << endl;
	}
  else{cout << "The re-start option is off" << endl; }
  ReadInput.close();
...
//Provo ad aprire il file old.0: se esiste, leggo le coordinate e le metto nelle variabili old;

  ReadConfOld.open("old.0");
  if(ReadConfOld.fail() or restart==false){
...
for (int idim=0; idim<3; ++idim) sumv[idim] /= (double)npart;
   ...
   fs = sqrt(3 * temp / sumv2);   // fs = velocity scale factor 
   for (int i=0; i<npart; ++i){
     vx[i] = fs * (Pbc(xnew[i] - xold[i])/(2.0 * delta));
     vy[i] = fs * (Pbc(ynew[i] - yold[i])/(2.0 * delta));
     vz[i] = fs * (Pbc(znew[i] - zold[i])/(2.0 * delta));

     //ricalcolo la configurazione attuale utilizzando le velocità riscalate
     xold[i] = Pbc(xnew[i] - vx[i] * delta);
     yold[i] = Pbc(ynew[i] - vy[i] * delta);
     zold[i] = Pbc(znew[i] - vz[i] * delta);
  ...
  for (int i=0; i<npart-1; ++i){
    for (int j=i+1; j<npart; ++j){
     ...
     for( int k=igofr; k<igofr+nbins; ++k){
	     if ( (k-4)*bin_size < dr & dr < (k-3)*bin_size ){
		walker[k]=walker[k]+2;
...
void ConfPreFinal(void){ //Write second-last configuration
  ofstream WriteConfPre;

  cout<<"Print second-last configuration to file old.final "<< endl << endl;
  WriteConfPre.open("old.final");

   for (int i=0; i<npart; ++i){
    WriteConfPre << x[i]/box << "   " <<  y[i]/box << "   " << z[i]/box << endl;
  }
  WriteConfPre.close();
  return;
}
...
void Averages(int iblk) //Print results for current block
{
  ...
  Gave.open("output.gave.0");
   ...
   stima_etot = blk_av[ie]/blk_norm;
    ...
    ave_Epot << setw(wd) << iblk <<  setw(wd) << stima_pot << setw(wd) << glob_av[iv]/(double)iblk << setw(wd) << err_pot << endl;

    ave_Ekin << setw(wd) << iblk <<  setw(wd) << stima_kin << setw(wd) << glob_av[ik]/(double)iblk << setw(wd) << err_kin << endl;

 
    ave_Temp << setw(wd) << iblk <<  setw(wd) << stima_temp << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_pot << endl;

    ave_Etot << setw(wd) << iblk <<  setw(wd) << stima_etot << setw(wd) << glob_av[ie]/(double)iblk << setw(wd) << err_etot << endl;

    //g(r)
    for(int k=4; k<104; k++){
	r = (k-4)*bin_size;
	//Gofr << r <<" "<< walker[k] <<endl;
	deltaV = (4./3.)*M_PI*(pow((k-3)*bin_size,3)-pow((k-4)*bin_size,3));
	stima_gdir=blk_av[k]/blk_norm/(rho*double(npart)*deltaV);
	glob_av[k] += stima_gdir;
        glob_av2[k] += stima_gdir*stima_gdir;
        err_gdir=Error(glob_av[k],glob_av2[k],iblk);
	Gofr << setw(wd) << r <<  setw(wd) << stima_gdir << endl;
